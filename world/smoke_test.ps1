...
try {
    ...
    Assert-HasPattern $bridgePath 'crossroad_rounds' 'Bridge crossroad_rounds status is missing'
    ...
    Assert-HasPattern $webAppPath 'function renderBranchTracks' 'Web branch progress tracks are missing'
    Assert-HasPattern $webAppPath 'live_branch_targets' 'Web live branch target HUD is missing'
    Assert-HasPattern $webAppPath 'crossroad: crossroads.has' 'Web crossroad track segments are missing'
