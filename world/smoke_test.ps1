...
try {
    ...
    Assert-HasPattern $optionsPath 'When enabled, in-page search' 'Searchsanity docstring is missing'
    Assert-HasPattern $optionsPath 'class Scrollsanity' 'Scrollsanity option is missing'
    Assert-HasPattern $optionsPath 'scrolling starts slow' 'Scrollsanity docstring is missing'
    Assert-HasPattern $optionsPath 'class SearchStartingLetters' 'Search Starting Letters option is missing'
    Assert-HasPattern $optionsPath 'all_vowels: A E I O U' 'SearchStartingLetters docstring must name the letter sets'
    Assert-HasPattern $optionsPath 'playable immediately at seed start' 'StartRoundsUnlocked docstring is missing'
    Assert-HasPattern $optionsPath 'each Round Access item unlocks' 'RoundsPerUnlock docstring is missing'
    Assert-HasPattern $optionsPath 'Allow video-game-tagged articles' 'IncludeVideoGames docstring is missing'
    ...
    Assert-HasPattern $bridgePath 'cmd == "RoomUpdate"' 'Bridge must apply room Release checks via RoomUpdate'
    Assert-HasPattern $bridgePath 'def _sync_progress_from_checked' 'Bridge must sync HUD progress from AP checked locations'
    ...
    Assert-HasPattern $webAppPath 'BINGO_SIDE_CELL_PX' 'Sidebar bingo cells must use a constant pixel size'
    Assert-HasPattern $webAppPath 'bingo-diag-tick' 'Bingo boards must mark diagonal and anti-diagonal'
    Assert-HasPattern $webAppPath 'function openJourneyPopup' 'Journey pop-out helper is missing'
    Assert-HasPattern $webAppPath 'function toggleJourneyFullscreen' 'Journey fullscreen helper is missing'
    Assert-HasPattern $webIndexPath 'id="journeyFullscreenBtn"' 'Journey fullscreen button is missing'
    Assert-HasPattern $webIndexPath 'id="journeyPopupBtn"' 'Journey pop-out button is missing'
    ...
    Assert-HasPattern $webIndexPath 'id="releaseBanner"' 'Web 1.0 release banner markup is missing'
    Assert-HasPattern $webIndexPath 'Support Wikipelago' 'Web Ko-fi button must say Support Wikipelago'
    Assert-HasPattern $webI18nPath 'banner.release10' 'i18n 1.0 release banner string is missing'
    ...
    Assert-HasPattern $webAppPath 'BINGO_SIDE_CELL_PX = 40.4' 'Sidebar bingo cells must stay at the 5x5 size'
